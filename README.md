...
| `:TrailBlazerMoveToNearest`                | `<buffer? string \| number>`<br>`<directive? string>`                                                          | Move to the nearest trail mark in the current or the nearest trail mark within the specified buffer. This calculates the minimum [Manhattan Distance][manhattan_distance] between the current cursor position and the qualifying trail marks. Passing one of the available [motion directives](#trailblazermovetonearest-directives) to this command will change the behavior of this motion. |
